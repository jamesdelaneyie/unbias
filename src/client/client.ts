...
const createPlayer = (entity: any, app: Application, entities: EntityMap) => {
    ...
    const username = new TaggedTextPlus(entity.username, {
        default: {
            fontSize: fontSize,
            fill: "#fff",
            align: "center",
...
const drawBasicText = (app: Application, masterContainer: Container, text: string, x: number, y: number) => {
...
window.addEventListener('load', async () => {
    ...
    const tick = (delta: number) => {
        ...
        if(keys.w || keys.a || keys.s || keys.d) {
            ...
            client.addCommand({ ntype: NType.Command, w: keys.w, a: keys.a, s: keys.s, d: keys.d, delta })
        }
